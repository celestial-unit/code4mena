...
export class LegalService {
  ...
  async getLegalUpdates(
    ...
    try {
      ...
      const updates = await apiService.get<any[]>(
        `/api/v1/legal/updates?${params.toString()}`
      );
  ...
  async getLegalCategories(
    language: string = 'ar'
  ): Promise<ApiResponse<LegalCategory[]>> {
    try {
      const categories = await apiService.get<LegalCategory[]>(
        `/api/v1/legal/categories?language=${language}`
      );
  ...
  async getLegalDocuments(
    ...
    try {
      ...
      const documents = await apiService.get<LegalDocument[]>(
        `/api/v1/legal/documents?${params.toString()}`
      );
  ...
  async getLegalDocumentById(
    ...
    try {
      const document = await apiService.get<LegalDocument>(
  ...
  async searchLegalDocuments(
    ...
    try {
      ...
      const results = await apiService.get<LegalSearchResult[]>(
  ...
  async getSearchSuggestions(
    ...
    try {
      ...
      const suggestions = await apiService.get<string[]>(
        `/api/v1/legal/search-suggestions?${params.toString()}`
      );
  ...
  async getMinistries(language: string = 'ar'): Promise<ApiResponse<any[]>> {
    try {
      const ministries = await apiService.get<any[]>(
        `/api/v1/legal/ministries?language=${language}`
      );
  ...
  async getLegalTags(
    ...
    try {
      ...
      const tags = await apiService.get<any[]>(
        `/api/v1/legal/tags?${params.toString()}`
      );
