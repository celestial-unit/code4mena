...
export const useMascotInteractions = () => {
  const { mascotState, updateSector, updateEmotion, triggerInteraction } =
    useMascot();
  ...
  const switchToSector = useCallback(
    (sector: MascotSector) => {
      updateSector(sector);
    },
    [updateSector]
  ...
  const reactToUserAction = useCallback(
    (action: 'success' | 'error' | 'help' | 'search' | 'chat') => {
      switch (action) {
        case 'success':
          showHappiness();
          break;
  ...
  const reactToSectorContent = useCallback(
    (
      contentType: 'legal_document' | 'case_study' | 'law_article' | 'procedure'
    ) => {
      switch (contentType) {
        case 'legal_document':
  ...
  const celebrateTunisianEvent = useCallback(
    (event: 'independence' | 'revolution' | 'ramadan' | 'eid') => {
