...
export class ChatService {
  ...
  private async simulateAIThinking(message: string): Promise<void> {
    ...
    const complexity =
      wordCount > 20 ? 'complex' : wordCount > 10 ? 'medium' : 'simple';
  ...
  async getQuickReplies(
    ...
    try {
      ...
      const quickReplies = await apiService.get<QuickReply[]>(
        `/api/v1/chat/quick-replies?${params.toString()}`
      );
  ...
  private createContextualResponse(message: string, category: LegalCategory) {
    ...
    if (category === 'business_law') {
      if (
        messageLower.includes('تسجيل') ||
        messageLower.includes('شركة') ||
        messageLower.includes('register')
      ) {
        return this.createBusinessRegistrationResponse();
      }
      if (
        messageLower.includes('ضريبة') ||
        messageLower.includes('tax') ||
        messageLower.includes('رقمي')
      ) {
        return this.createDigitalTaxResponse();
      }
      if (
        messageLower.includes('تجارة إلكترونية') ||
        messageLower.includes('e-commerce')
      ) {
    ...
    if (category === 'tax_law') {
      if (
        messageLower.includes('قيمة مضافة') ||
        messageLower.includes('vat') ||
        messageLower.includes('tva')
      ) {
      ...
      if (
        ...
        messageLower.includes('échéance')
      ) {
    ...
    if (category === 'labor_law') {
      if (
        messageLower.includes('أجر') ||
        messageLower.includes('wage') ||
        messageLower.includes('salaire')
      ) {
        return this.createWageResponse();
      }
      if (
        messageLower.includes('عن بُعد') ||
        messageLower.includes('remote') ||
        messageLower.includes('télétravail')
      ) {
    ...
    if (category === 'administrative_law') {
      if (
        messageLower.includes('رخصة') ||
        messageLower.includes('permit') ||
        messageLower.includes('permis')
      ) {
        return this.createPermitResponse();
      }
      if (
        messageLower.includes('عضوي') ||
        messageLower.includes('organic') ||
        messageLower.includes('biologique')
      ) {
    ...
    if (category === 'civil_law') {
      if (
        messageLower.includes('زواج') ||
        messageLower.includes('marriage') ||
        messageLower.includes('mariage')
      ) {
        return this.createMarriageResponse();
      }
      if (
        messageLower.includes('وراثة') ||
        messageLower.includes('inheritance') ||
        messageLower.includes('héritage')
      ) {
    ...
    if (category === 'family_law') {
      ...
      if (
        messageLower.includes('حضانة') ||
        messageLower.includes('custody') ||
        messageLower.includes('garde')
      ) {
      ...
      legalReferences: [
        {
          ...
          excerptFr:
            "Toutes les entreprises doivent s'enregistrer auprès du CFE",
        },
      ],
      suggestedActions: [
        {
        ...
        culturalReferences: [
          {
            ...
            explanationFr:
              "Guichet unique pour l'enregistrement d'entreprise en Tunisie",
      ...
      legalReferences: [
        {
          ...
          excerptFr:
            'Réglementations pour les opérations commerciales en ligne',
        },
      ],
      suggestedActions: [
        {
          ...
          descriptionFr:
            'Apprendre les droits des consommateurs dans le e-commerce',
  ...
  private createGeneralBusinessResponse() {
    return {
      content:
        "I'm here to help with your business law questions in Tunisia...",
      contentAr:
        'أنا هنا لمساعدتك في أسئلة قانون الأعمال في تونس! يمكنني مساعدتك في:\n\n**🏢 تأسيس الشركات:**\n- أنواع الشركات المختلفة\n- متطلبات التسجيل\n- رأس المال المطلوب\n\n**📄 الامتثال القانوني:**\n- اللوائح التجارية\n- التراخيص المطلوبة\n- الالتزامات الضريبية\n\n**👥 قانون العمل:**\n- عقود العمل\n- حقوق الموظفين\n- إجراءات التوظيف\n\n**💼 العقود التجارية:**\n- صياغة العقود\n- شروط البيع\n- اتفاقيات الشراكة\n\nما هو السؤال المحدد الذي تريد مساعدة فيه؟',
      contentFr:
        'Je suis là pour vous aider avec vos questions de droit des affaires en Tunisie...',
      confidence: 0.85,
  ...
  > {
    try {
      ...
      const aiMessage: ChatMessage = {
        ...
        contentAr:
          response.aiMessage?.contentAr || response.aiMessage?.content || '',
  ...
  async getUserConversations(
    ...
    try {
      ...
      const conversations = await apiService.get<ChatConversation[]>(
  ...
  async createConversation(
    ...
    try {
      const conversation = await apiService.post<ChatConversation>(
  ...
  async getConversationById(
    ...
    try {
      const conversation = await apiService.get<ChatConversation>(
    ...
    try {
      ...
      return {
        ...
        error: {
          ...
          messageFr:
            'Échec de la récupération des statistiques de conversation',
        },
        timestamp: new Date(),
