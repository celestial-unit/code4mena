  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.detail && data.detail.api) {
        ...
        const services = ['api', 'database', 'legal_rag', 'external_llm', 'audio_service'];
        services.forEach(service => {
          if (data.detail[service]) {
            const isHealthy = data.detail[service].includes('healthy');
            logTest(`${service} service status`, isHealthy, data.detail[service]);
          }
        });
      } else {
        logTest('Health response has correct structure', false, 'Missing expected fields');
      }
    } else {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.response && data.sources && data.disclaimer) {
        ...
        logTest('Legal query returns sources', data.sources.length > 0, `Found ${data.sources.length} sources`);
      } else {
        logTest('Legal query response has correct structure', false, 'Missing expected fields');
      }
    } else {
      const errorText = await response.text();
      logTest('Legal query endpoint accessible', false, `HTTP ${response.status}: ${errorText}`);
    }
  } catch (error) {
...
async function testPopularQueriesApi() {
  ...
  try {
    const response = await makeRequest(`${API_BASE_URL}/popular-queries?language=ar`);
      ...
      if (Array.isArray(data)) {
        ...
        if (data.length > 0) {
          ...
          if (firstQuery.query && typeof firstQuery.popularity === 'number') {
            ...
            logTest('Popular queries have correct structure', false, 'Missing expected fields');
          }
        }
      } else {
        logTest('Popular queries returns array', false, 'Response is not an array');
      }
    } else {
      const errorText = await response.text();
      logTest('Popular queries endpoint accessible', false, `HTTP ${response.status}: ${errorText}`);
    }
  } catch (error) {
...
async function testAuthenticationTokenHandling() {
  ...
  try {
    ...
    if (response.ok) {
      logTest('API works without token (expected for current implementation)', true);
    } else {
...
async function testErrorHandling() {
  ...
  try {
    ...
    if (!response.ok) {
      logTest('API returns proper error for invalid endpoint', true, `HTTP ${response.status}`);
      ...
      try {
        ...
        if (errorData.detail || errorData.message) {
          ...
          logTest('Error response has proper structure', false, 'No error details provided');
        }
      } catch {
        logTest('Error response has proper structure', false, 'Error response is not JSON');
      }
    } else {
      logTest('API returns proper error for invalid endpoint', false, 'Should return error for invalid endpoint');
    }
  } catch (error) {
    logTest('API returns proper error for invalid endpoint', false, error.message);
  }
  ...
  try {
    ...
    if (!response.ok) {
      logTest('API handles malformed requests properly', true, `HTTP ${response.status}`);
    } else {
      logTest('API handles malformed requests properly', false, 'Should return error for malformed JSON');
    }
  } catch (error) {
...
async function testTimeoutHandling() {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      ...
      logTest('Timeout handling works', false, `Unexpected error: ${error.message}`);
    }
  }
}
...
async function testApiServiceIntegration() {
  ...
  logTest('API base URL configuration', API_BASE_URL === expectedBaseUrl, `Expected: ${expectedBaseUrl}, Got: ${API_BASE_URL}`);
  ...
  try {
    ...
    if (response.ok) {
      ...
      const errorText = await response.text();
      logTest('Backend accepts frontend request format', false, `HTTP ${response.status}: ${errorText}`);
    }
  } catch (error) {
