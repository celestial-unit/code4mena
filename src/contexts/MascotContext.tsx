...
export const MascotProvider: React.FC<MascotProviderProps> = ({ children }) => {
  const [mascotState, setMascotState] = useState<MascotState>(defaultMascotState);
    ...
    try {
      ...
      if (savedState) {
        ...
        setMascotState({
          ...
          lastInteraction: parsedState.lastInteraction ? new Date(parsedState.lastInteraction) : null
        });
      }
    } catch (error) {
