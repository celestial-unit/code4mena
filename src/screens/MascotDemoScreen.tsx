...
export const MascotDemoScreen: React.FC<MascotDemoScreenProps> = ({ navigation }) => {
  ...
  const demoAchievement = {
    ...
    descriptionEn: 'You have successfully completed your first legal consultation!',
    icon: 'trophy',
  ...
  const emotionButtons = [
    { emotion: 'greeting', label: 'ترحيب', icon: 'hand-left-outline', action: greetUser },
    { emotion: 'thinking', label: 'تفكير', icon: 'bulb-outline', action: showThinking },
    { emotion: 'happy', label: 'سعادة', icon: 'happy-outline', action: showHappiness },
    { emotion: 'excited', label: 'حماس', icon: 'flash-outline', action: showExcitement },
    { emotion: 'concerned', label: 'قلق', icon: 'alert-circle-outline', action: showConcern },
    { emotion: 'explaining', label: 'شرح', icon: 'chatbubble-outline', action: explainConcept },
  ];

  const actionButtons = [
    { action: 'success', label: 'نجاح', icon: 'checkmark-circle', color: theme.colors.success },
    { action: 'error', label: 'خطأ', icon: 'close-circle', color: theme.colors.error },
    { action: 'help', label: 'مساعدة', icon: 'help-circle', color: theme.colors.info },
    { action: 'search', label: 'بحث', icon: 'search', color: theme.colors.accent },
    { action: 'chat', label: 'محادثة', icon: 'chatbubble', color: theme.colors.primary },
  ];
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {/* Mascot Display */}
        <View style={[styles.mascotSection, { backgroundColor: theme.colors.surface }]}>
          ...
          <View style={styles.statusContainer}>
            ...
            <View style={[styles.statusItem, { backgroundColor: theme.colors.background }]}>
              ...
              <Text style={[styles.statusValue, { color: isAnimating ? theme.colors.success : theme.colors.textSecondary }]}>
                {isAnimating ? 'نعم' : 'لا'}
              </Text>
            </View>
          </View>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface }]}>
          ...
          <View style={styles.buttonGrid}>
            {emotionButtons.map((button, index) => (
              <TouchableOpacity
                key={index}
                style={[styles.emotionButton, { backgroundColor: theme.colors.background }]}
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface }]}>
          ...
          <View style={styles.buttonGrid}>
            {culturalEvents.map((event, index) => (
              <TouchableOpacity
                key={index}
                style={[styles.culturalButton, { backgroundColor: theme.colors.background }]}
